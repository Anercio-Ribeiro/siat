...
/**
 * DashboardInquilinoProprietarioFlipBox is a React functional component that renders a dynamic dashboard
 * for users with the roles "PROPRIETARIO" (Owner) or "INQUILINO" (Tenant). The dashboard displays
 * various statistics, charts, and key performance indicators (KPIs) relevant to the user's role and
 * selected rental type ("RESIDENCIAL" or "TURISTICO").
 *
 * Features:
 * - Role-based dashboard: Renders different sets of cards and charts for proprietários and inquilinos.
 * - Flip animation and loading state when toggling between rental types.
 * - Year and month filters for dashboard data.
 * - Responsive and resizable chart cards (Bar, Line, Pie charts) using Chart.js.
 * - Displays trend mini-charts and highlights positive/negative values with color cues and icons.
 * - Uses React Query for data fetching and caching.
 * - Skeleton loading UI while fetching data.
 *
 * Dependencies:
 * - React, React Query, Chart.js (react-chartjs-2), and custom UI components (Card, Button, Skeleton, etc.).
 *
 * @returns {JSX.Element} The rendered dashboard component for the current user.
 */
