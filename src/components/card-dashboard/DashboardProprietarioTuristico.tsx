...
// export const DashboardProprietario: React.FC = () => {
//   const { user } = useUser();

//   const { data, isLoading } = useQuery({
//     queryKey: ["dashboard-proprietario", user?.id],
//     queryFn: () => fetch("/api/dashboard/proprietario").then(res => res.json()),
...
//   return (
//     <div className="p-4 space-y-6">
//       <h1 className="text-2xl font-bold">Dashboard do Proprietário</h1>
...
export const DashboardProprietarioTuristico: React.FC = () => {
  const { user } = useUser();

  const { data, isLoading } = useQuery({
    queryKey: ["dashboard-proprietario-turistico", user?.id],
    queryFn: () => fetch("/api/dashboard/proprietario?tipoAluguel=TURISTICO").then(res => res.json()),
    enabled: !!user && user.role === "PROPRIETARIO",
  });
  ...
  const barChartData = {
    ...
    datasets: [
      {
        label: "Percentual (%)",
        data: [data.taxaOcupacao ?? 0, data.comparacaoMedia ?? 0], // Valores padrão caso undefined
        backgroundColor: ["rgba(75, 192, 192, 0.6)", "rgba(255, 99, 132, 0.6)"],
        borderColor: ["rgba(75, 192, 192, 1)", "rgba(255, 99, 132, 1)"],
        borderWidth: 1,
      },
  ...
  return (
    <div className="p-4 space-y-6">
      <h1 className="text-2xl font-bold">Dashboard do Proprietário - Turístico</h1>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <Card>
          <CardHeader>
            <CardTitle>Taxa de Ocupação</CardTitle>
