...
// export const DashboardInquilino: React.FC = () => {
//   const { user } = useUser();

//   const { data, isLoading } = useQuery({
//     queryKey: ["dashboard-inquilino", user?.id],
//     queryFn: () => fetch("/api/dashboard/inquilino").then(res => res.json()),
//     enabled: !!user && user.role === "INQUILINO",
//   });

//   if (!user) return <div>Carregando usuário...</div>;
//    if (user.role !== "INQUILINO") return <div></div>;
...
//   return (
//     <div className="p-4 space-y-6">
//       <h1 className="text-2xl font-bold">Dashboard do Inquilino</h1>
...
export const DashboardInquilinoTuristico: React.FC = () => {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => fetch("/api/dashboard/inquilino?tipoAluguel=RESIDENCIAL").then(res => res.json()),
    enabled: !!user && user.role === "INQUILINO",
  });
  ...
  return (
    <div className="p-4 space-y-6">
      <h1 className="text-2xl font-bold">Dashboard do Inquilino - Residencial</h1>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <Card>
          <CardHeader>
            <CardTitle>Imóveis Disponíveis</CardTitle>
