...
//                       <p>
//                          <strong>Preço:</strong> {(imovel.precoMensal ?? 0).toLocaleString("pt-BR", { style: "currency", currency: "AOA" })} / mês
//                       </p>
...
//     preco: imovel.preco,
//     precoMensal: imovel.precoMensal ?? undefined,
...
//   const totalPages = Math.ceil(proximidades.length / itemsPerPage);
//   const startIndex = (currentPage - 1) * itemsPerPage;
//   const paginatedProximidades = proximidades.slice(startIndex, startIndex + itemsPerPage);
...
//                     <div className="flex items-center space-x-2">
//                       <p>
//                         <strong>Preço:</strong> {imovel.preco.toLocaleString("pt-BR", { style: "currency", currency: "AOA" })} / dia
//                       </p>
//                     </div>
//                     <div className="flex items-center space-x-2">
//                       <Home className="h-5 w-5 text-primary text-blue-600" />
...
//                     titulo={imovel.titulo}
//                     preco={imovel.preco}
//                     precoMensal={imovel.precoMensal || 0}
//                   />
//                   <div className="grid grid-cols-2 gap-4 mt-4">
//                     {/* Latitude and Longitude display commented out as in original */}
//                   </div>
//                 </div>
//               )}
//             </TabsContent>
...
//                   />
//                   <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
//                     {paginatedProximidades.map((proximidade: Proximidade) => (
//                       <div 
//                         key={proximidade.id} 
//                         className="p-3 border rounded-lg shadow-sm hover:shadow-md transition-shadow"
//                       >
//                         <p className="font-medium text-sm">{proximidade.nome}</p>
//                         <p className="text-xs text-gray-600">{proximidade.tipo}</p>
//                         <p className="text-xs text-gray-500 mt-1">
//                           {proximidade.calculated_distance.toFixed(2)} km de distância
//                         </p>
//                       </div>
...
//                     )}
//                   </div>
//                   {proximidades.length > itemsPerPage && (
...
const DialogContentComponent = ({ isLoading, imovel }: { 
  ...
  // Format tipo to replace underscores with spaces
  const formatTipo = (tipo: string): string => {
    return tipo ? tipo.replaceAll('_', ' ') : '';
  };
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
                    {paginatedProximidades.map((proximidade: Proximidade) => (
                      ...
                      >
                        <p className="font-medium text-sm">{proximidade.nome}</p>
                        <p className="text-xs text-gray-600">{formatTipo(proximidade.tipo)}</p>
                        <p className="text-xs text-gray-500 mt-1">
                          {proximidade.calculated_distance.toFixed(2)} km de distância
                        </p>
                      </div>
