...
//         console.log("Fetching proximidade with tipo:", tipo, "page:", pageNum);
...
//   // Fetch all imoveis on initial load
//   useEffect(() => {
//     if (!userLoading && initialLoading) {
//       handleSearch(1); // Load page 1 of all imoveis
//     }
...
//     handleSearch(1); // Reload all imoveis when filters are cleared
//   };
...
//   // Transforma o retorno da API no formato esperado pelo componente
//   const transformedData: FetchImoveisResponse = {
//     imoveis: data.Proximidade, // Os imóveis estão dentro da chave "Proximidade"
//     totalImoveis: data.Proximidade.length, // Ajuste conforme a API fornecer o total real
//     totalPages: Math.ceil(data.Proximidade.length / 8), // Calcula páginas com base no tamanho do array
//     currentPage: page,
//   };

//   // Simula delay de 3 segundos para loading
//   await new Promise((resolve) => setTimeout(resolve, 3000));
...
async function fetchProximidadeImoveis(tipo: string, page: number, distanciaMax?: string): Promise<FetchImoveisResponse> {
  ...
  return data; // Retorna diretamente os dados da API, que já estão no formato correto
}
